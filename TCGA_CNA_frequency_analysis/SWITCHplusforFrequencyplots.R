## plot.freq.SW <- function(data="Mouse-CN-28264_final_X1-orderCombine_SWITCHdna_out_F11_a10-OnlySig.txt", grps="Book_updateMouse_p001_062110_final.txt", species="hmChrLengthsFinal.txt", returnSummary = F)
#  
#  data = segmented CN data matrix 
#  grps = sample names and their subtype classifications
#  species = species-specific genome chromosome lengths
#  returnSummary = if TRUE prints summary to the screen
#
#  Using only the significant segmented copy number data to create a copy number frequency landscape plot
plot.freq.SW <- function(data, grps, species, threshold = 0, returnSummary = F){
  ...
  for(k in 1:length(cls.names)){
    ...
    if(nrow(x) > 0){
      ...
      pdf(file = paste(cls.names[k],"_SWITCHdna_landscape.pdf",sep=""), width = 18, height = 5)
      barplot((chr.gains/nsamples),chr.gains.length,space=0,col="#E41A1C",border=NA,ylim=c(-1,1),main=cls.names[k])
      barplot((-1*chr.losses/nsamples),chr.losses.length,space=0,col="#377EB8",border=NA,add=T)
      ...
      fileNameGains <- paste(cls.names[k], "_FrequencyCNAnnotationGains.txt", sep="")
      fileNameLoss <-  paste(cls.names[k], "_FrequencyCNAnnotationLoss.txt", sep="")
      tempTablegains <- cbind(chr.gains.chr, chr.gains.start, chr.gains.stop,(chr.gains/nsamples), chr.gains.length)
      tempTableloss <- cbind(chr.losses.chr, chr.losses.start, chr.losses.stop, (chr.losses/nsamples), chr.losses.length)
      write.table(tempTablegains,fileNameGains, sep="\t", quote=FALSE, col.names=TRUE, row.names=FALSE, na="")
      write.table(tempTableloss,fileNameLoss, sep="\t", quote=FALSE, col.names=TRUE, row.names=FALSE, na="")
